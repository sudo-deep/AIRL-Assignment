...
for img_path in img_list:
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            # Update the extreme coordinates for enclosing rectangle
            for point in approx:
                x, y = point[0]
                min_x = min(min_x, x)
                max_x = max(max_x, x)
    ...
    if found_rectangle:
        ...
        # Draw the enclosing rectangle on the blank image
        cv2.rectangle(blank_image, bottom_left, top_right, (255, 0, 0), 2)
